...
header_function void khadiSetMainCPU    (Khadi *k, Uint cpu) { k->main_cpu = cpu; }
header_function void khadiAddTaskCPU (Khadi *k, Uint cpu) { sbufAdd(k->task_cpus, cpu); }
header_function void khadiAddDataCPU    (Khadi *k, Uint cpu, Uint thread_count)
{
...
header_function void khadiAddCoroutines (Khadi *k, Size stack_size, Size count)
{
...
header_function
void* khadi_TaskFunction (void *arg) {
                     ...
                     Khadi_Thread_Function *task_func, Khadi_Thread_Function *data_func)
{
    ...
    { // Create Task and Data threads
        for (Size i = 0; i < sbufElemin(khadi->task_cpus); i++) {
            ...
            pthread_t thread;
            pthread_create(&thread, &attr, khadi_TaskFunction, (void*)task_func);
